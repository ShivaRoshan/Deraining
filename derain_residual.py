...
class Dense_rain_residual(nn.Module):
    ...
    def forward(self, x, label_d):
        ...
        label_d11 = Variable(label_d11.to(torch.device('cpu')))
        ...
        #raise Exception(label_d)

        # raise Exception(f"x1 {x1} shape {x1.size()} \n x2 {x2} shape {x2.size()}\n x3 {x3} shape {x3.size()}\n x {x} shape {x.size()} \n label_d11 {label_d11} shape {label_d11.size()}")
        # raise Exception(len(list(label_d11.size())))
        if len(list(label_d11.size())) == 1:
            # if label_d11.size(1) != 512:
            # label_d11 = label_d11.unsqueeze(1).expand(-1, 512)
            label_d11 = label_d11.unsqueeze(1).unsqueeze(2).unsqueeze(3).expand(-1, 4, 512, 512)
            # raise Exception(f"tensor\n{label_d11} size {label_d11.size()}")
            # label_d11.view(label_d11.size(0), 1, 1, 1) # Assuming 4D tensors, add dimensions
