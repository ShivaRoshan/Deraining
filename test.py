...
if torch.cuda.is_available():
	...
	# Load on CPU if CUDA is not available
	net_label.load_state_dict(torch.load('./classification/netG_epoch_9.pth', map_location=torch.device('cpu')))
	net_label = net_label.cpu()
...
if torch.cuda.is_available():
	...
	# Load on CPU if CUDA is not available
	residue_net.load_state_dict(torch.load('./residual_heavy/netG_epoch_6.pth', map_location=torch.device('cpu')))
	residue_net = residue_net.cpu()
...
for epoch in range(1):
    ...
    for i, data in enumerate(valDataloader, 0):
        if 1:
            ...
            val_target_cpu, val_input_cpu = val_target_cpu.float(), val_input_cpu.float()
